...
- 9d4e193: 修复第二轮 review 发现的问题，并让 HLS/DASH 引擎开箱即用：
...
- 3c0c468: feat(core): 右键菜单新增"复制视频地址"，默认复制当前页面地址，可用 `shareUrl` 选项覆盖，复制后有 OSD 通知反馈

## 1.4.0

### Minor Changes

- 0438852: **播放进度改用 sessionStorage 存储，并大幅提升记录精度。**
...
- d51705d: **新增：把偏好与播放进度存到自己的后端**，用于多端同步场景（`localStorage` 是分浏览器的，手机上的进度换电脑就没了）。
...
- 37ce690: **修复设置面板开关行的点击范围**：此前整行（含标签文字与空白区域）都会触发开关切换，容易误触。现在只有开关控件本身响应点击。开关行与滑块行的 `cursor` 也相应改为 `default`，开关本身保持 `pointer`，鼠标形态即可区分可点区域。
