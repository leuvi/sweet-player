...
- **Persistence**: Volume/playback rate stored in localStorage; pass `id` and resume position is kept in sessionStorage for the current tab. To outlive the tab or sync across devices, route it through your own backend with `onSavePrefs` / `onSaveProgress` + `player.restore()` — throttling, save timing and failure handling stay inside the player
